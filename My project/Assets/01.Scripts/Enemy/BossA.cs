...
using System.Collections.Generic; // 코루틴 쓸 때 필요한 using문
...
public class BossA : MonoBehaviour
{
	...
	private void NextPattern()
	{
		...
		_currentPatternIndex = (_currentPatternIndex + 1) % 9;
	...
	private void MoveSideways()
	{
		if (_movingRight)
		{
			...
			if (transform.position.x > MoveDistance) // 현재 위치의 x값이 > MoveDistance 이면
			{
	...
	private void Pattern1()
	{
		// 패턴 1: 원형으로 총알 발사
		int numBullets1 = 30;
		float angleStep1 = 360.0f / numBullets1; // 현재 각도를 numBullets1으로 나눔 (각도에 따라 나눠서 발사되게 하려고)
	...
	private void Pattern2()
	{
		// 패턴 2: 방사형으로 총알 발사
		int numBullets2 = 12;
		float angleStep2 = 360.0f / numBullets2; // 현재 각도를 numBullets2으로 나눔 (각도에 따라 나눠서 발사되게 하려고)
	...
	private void Pattern6()
	{
		...
		// 각 총알의 간격 각도 계산
		float angleStep = 180.0f / numBullets;
